...
from farchive import Farchive, PathLike
...
def _parse_timestamp(value: str) -> datetime:
    ...
    'Z' suffix is always interpreted as UTC.
    """
    # Normalize Z suffix to +00:00 before any parsing attempt
    if value.endswith("Z"):
        value = value[:-1] + "+00:00"

    # Try tz-aware ISO 8601 formats
    for fmt in (
        "%Y-%m-%dT%H:%M:%S.%f%z",
        "%Y-%m-%dT%H:%M:%S%z",
        "%Y-%m-%d %H:%M:%S%z",
    ...
    # Naive formats — interpret as local time
    for fmt in (
        "%Y-%m-%dT%H:%M:%S.%f",
        "%Y-%m-%dT%H:%M:%S",
        "%Y-%m-%d %H:%M:%S",
...
def _is_valid_db(path: PathLike) -> bool:
...
def _open_ro(args: argparse.Namespace) -> "Farchive":
    """Open archive in read-only mode, with strict schema verification."""
    import sqlite3
    from pathlib import Path
    ...
    if version < SCHEMA_VERSION:
        if version == 0:
            print("Error: Not a valid farchive database or missing schema info.", file=sys.stderr)
        else:
            print(f"Error: Archive schema version ({version}) is too old. Run 'farchive migrate {args.db}' first.", file=sys.stderr)
        sys.exit(1)
    elif version > SCHEMA_VERSION:
        print(f"Error: Archive schema version ({version}) is newer than library supports ({SCHEMA_VERSION}).", file=sys.stderr)
        sys.exit(1)

    return Farchive(args.db, readonly=True)
...
def _cmd_stats(args: argparse.Namespace) -> None:
    ...
    print(f"Generated by:     {st.generator}")
...
def _cmd_du(args: argparse.Namespace) -> None:
    ...
    with _open_ro(args) as fa:
        ...
        if by == "storage-class":
            ...
            # Unique chunk bytes per storage class (DISTINCT chunk_digest avoids overcounting shared chunks)
            chunk_by_sc: dict = {}
            for sc_row in fa._conn.execute(
                "SELECT COALESCE(uniq.storage_class, '(none)') as sc, "
                "SUM(c.stored_size) as sz "
                "FROM chunk c "
                "JOIN ("
                "  SELECT DISTINCT bc.chunk_digest, b2.storage_class "
                "  FROM blob_chunk bc JOIN blob b2 ON bc.blob_digest = b2.digest "
                "  WHERE b2.codec = 'chunked'"
                ") uniq ON c.chunk_digest = uniq.chunk_digest "
                "GROUP BY uniq.storage_class"
            ).fetchall():
                chunk_by_sc[sc_row[0]] = sc_row[1] or 0
            if not args.json:
                ...
                if chunk_by_sc:
                    print("  Note: chunked blob bytes attributed to owning storage class (unique chunks only)")
            else:
                items = [
                    {
                        "storage_class": r["sc"],
            ...
            # Add unique chunk bytes for chunked codec (DISTINCT to avoid overcounting)
            chunk_total = fa._conn.execute(
                "SELECT COALESCE(SUM(c.stored_size), 0) FROM chunk c"
            ).fetchone()[0]
            if not args.json:
                ...
                print("-" * 52)
                for r in rows:
                    extra = chunk_total if r["codec"] == "chunked" else 0
                    print(
                        f"{r['codec']:<16} {r['cnt']:>8,} {r['raw']:>12,} {r['stored'] + extra:>12,}"
                    )
            else:
                items = [
                    {
                        ...
                        "blobs": r["cnt"],
                        "raw": r["raw"],
                        "stored": r["stored"] + (chunk_total if r["codec"] == "chunked" else 0),
                    }
                    for r in rows
                ]
                print(_json_dumps(items, indent=2))
            ...
            # Distinct chunk bytes uniquely utilized per locator
            chunk_by_loc: dict = {}
            for loc_row in fa._conn.execute(
                "SELECT uniq.locator, SUM(c.stored_size) as sz "
                "FROM chunk c "
                "JOIN ("
                "  SELECT DISTINCT ls2.locator, bc.chunk_digest "
                "  FROM locator_span ls2 "
                "  JOIN blob_chunk bc ON ls2.digest = bc.blob_digest"
                ") uniq ON c.chunk_digest = uniq.chunk_digest "
                "GROUP BY uniq.locator"
            ).fetchall():
                chunk_by_loc[loc_row[0]] = loc_row[1] or 0

            if not args.json:
                print(f"{'locator':<40} {'blobs':>6} {'raw':>12} {'stored':>12}")
                print("-" * 76)
                for r in rows[: args.top]:
                    extra = chunk_by_loc.get(r["locator"], 0)
                    total_stored = r["stored"] + extra
                    print(
                        f"{r['locator']:<40} {r['blobs']:>6,} {r['raw']:>12,} {total_stored:>12,}"
                    )
                if args.top and len(rows) > args.top:
                    print(f"  ... and {len(rows) - args.top} more")
                if chunk_by_loc:
                    print("  Note: chunk bytes distinct to each locator are included (may overlap across locators)")
            else:
                items = [
                    {
                        "locator": r["locator"],
                        "blobs": r["blobs"],
                        "raw": r["raw"],
                        "stored": r["stored"] + chunk_by_loc.get(r["locator"], 0),
                    }
                    for r in rows
                ]
                print(_json_dumps(items, indent=2))
...
def _cmd_diff(args: argparse.Namespace) -> None:
    ...
    _ensure_db(args)
    with _open_ro(args) as fa:
        # Determine ref_a: explicit flag > positional > error
        if args.digest_a:
            ...
            ref_a = args.ref_a
            is_64_hex_a = len(ref_a) == 64 and all(
                c in "0123456789abcdef" for c in ref_a.lower()
            )
            if is_64_hex_a:
                print(
                    f"Error: ambiguous ref_a '{ref_a[:16]}...'. "
                    f"Use --digest-a or --locator-a explicitly.",
                    file=sys.stderr,
                )
                sys.exit(1)
            is_digest_a = False
        ...
        # Determine ref_b: explicit flag > positional > error
        if args.digest_b:
            ...
            ref_b = args.ref_b
            is_64_hex_b = len(ref_b) == 64 and all(
                c in "0123456789abcdef" for c in ref_b.lower()
            )
            if is_64_hex_b:
                print(
                    f"Error: ambiguous ref_b '{ref_b[:16]}...'. "
                    f"Use --digest-b or --locator-b explicitly.",
                    file=sys.stderr,
                )
                sys.exit(1)
            is_digest_b = False
...
def _cmd_migrate(args: argparse.Namespace) -> None:
    ...
    if current == SCHEMA_VERSION:
        ...
        print(
            f"Error: Archive schema version ({current}) is newer than library supports ({SCHEMA_VERSION}).",
            file=sys.stderr,
        )
        sys.exit(1)

    print(f"Migrating schema from version {current} to {SCHEMA_VERSION}...", file=sys.stderr)
    # The normal Farchive init will perform the migration automatically and save it
    with Farchive(args.db):
        pass

    import sqlite3
    from farchive._schema import detect_schema_version
    with sqlite3.connect(db_uri, uri=True) as conn:
        after = detect_schema_version(conn)

    if after != SCHEMA_VERSION:
        print(f"Error: Migration failed. Version is still {after}.", file=sys.stderr)
        sys.exit(1)

    print(f"Migration successful. Version is now {after}.", file=sys.stderr)
...
def main(argv: list[str] | None = None) -> None:
    ...
    p.add_argument("db", help="DB path")
    p.add_argument("ref_a", nargs="?", default=None, help="First locator or digest")
    p.add_argument("--locator-a", default=None, help="Explicit first locator")
    p.add_argument("--digest-a", default=None, help="Explicit first digest")
    p.add_argument("ref_b", nargs="?", default=None, help="Second locator or digest")
    p.add_argument("--locator-b", default=None, help="Explicit second locator")
    p.add_argument("--digest-b", default=None, help="Explicit second digest")
